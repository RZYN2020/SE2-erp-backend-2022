...
@SpringBootTest
public class SaleReturnServiceTest {
  ...
  public void makeSaleSheet() { // 测试销售单是否生成成功
    ...
    String realSheetId = IdGenerator
        .generateSheetId(prevSheet == null ? null : prevSheet.getId(), "XSD");
  ...
  @Transactional
  @Rollback(value = true)
  public void makeSalesReturnSheet() {
    //先制作一个销售单
    makeSaleSheet();
    ...
    UserVO userVO = UserVO.builder()
        ...
        .role(Role.SALE_MANAGER)
        .build();
    List<SaleReturnSheetContentVO> contentVOS = new ArrayList<>();
    contentVOS.add(SaleReturnSheetContentVO.builder()
        .pid("0000000000400000")
        .quantity(50)
        .remark("Test1-product1")
        .build());
    ...
    SaleReturnSheetVO saleReturnSheetVO = SaleReturnSheetVO.builder()
        .saleReturnSheetContent(contentVOS)
        .saleSheetID(saleSheetDao.getLatestSheet().getId())
        .remark("Test1")
        .build();
    ...
    String sheetID = latestSheet.getId();
    Assertions.assertNotNull(sheetID);
    List<SaleReturnSheetContentPO> content = saleReturnSheetDao.findContentBySaleReturnSheetId(sheetID);
  ...
  public void approval_2() { // 测试二级审批
    ...
    Assertions.assertEquals(0, productDao.findById("0000000000400000").getRecentRp().compareTo(
    ...
    List<WarehouseOutputSheetPO> draftSheets = warehouseOutputSheetDao.getDraftSheets(
        WarehouseOutputSheetState.DRAFT);

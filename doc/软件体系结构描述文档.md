...
![后端包图](软件体系结构描述文档.assets/后端包图.png)
...
业务逻辑层包括多个针对界面的业务逻辑处理对象。例如`Inventory`负责处理库存界面的业务逻辑；`Sales`负责处理销售界面的业务逻辑；`Financial`负责处理财务界面的业务逻辑；`HR`负责处理人力资源界面的业务逻辑
...
| financialbl | 负责实现财务界面的业务逻辑         |
| HRbl        | 负责处理人力资源界面的业务逻辑     |
|             |                                    |
...
库存模块的业务逻辑层需要的服务由InventoryDao接口提供，具体描述如下图所示。
...
包含employee表、job表和salary_sheet表